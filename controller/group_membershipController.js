  ...
  try {
    ...
    if (!req.user || req.user.id === undefined) {
        return res.status(401).json({
            success: false,
            message: "Unauthorized: member_id is required",
        });
    }

    // Check if the group already exists
    const [group] = await db.promise().query('SELECT * FROM `group` WHERE id = ?', [group_id]);
    if (group.length === 0) {
        return res.status(400).json({ message: 'Invalid group_id: not found' });
    }
    
    // check if the membership already exists
    const [existingMembership] = await db.promise().query('SELECT * FROM group_membership WHERE group_id = ? AND member_id = ?',[group_id, req.user.id]);
    if (existingMembership.length > 0) {
        return res.status(409).json({ message: 'Membership already exists' });
    }

    const id = uuidv4();
    const sql = 'INSERT INTO group_membership (id, group_id, member_id) VALUES (?, ?, ?)';
    const [result] = await db.promise().query(sql, [id, group_id, req.user.id]);
    res.status(201).json({ id: id, group_id, member_id: req.user.id });
...
exports.getAllGroupMemberships = async (req, res) => {
    try {
        const [rows] = await db.promise().query('SELECT * FROM group_membership');
        res.json(rows);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
exports.getGroupMembershipById = async (req, res) => {
  try {
    ...
    const [rows] = await db.promise().query('SELECT * FROM group_membership WHERE id = ?', [id]);
